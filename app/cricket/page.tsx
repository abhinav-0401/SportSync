...
export default function Cricket() {
    ...
    const fetchData = async () => {
        try {
            ...
            setFilteredData(response?.data?.matchScheduleMap); // Initialize with all matches
            setError(null);
        } catch (error) {
    ...
    const filterData = (type: string) => {
        ...
        if (type === 'All') {
            setFilteredData(data?.matchScheduleMap);
        } else {
            const filtered = data?.matchScheduleMap?.map((schedule: any) => ({
                ...schedule,
                scheduleAdWrapper: {
                    ...schedule?.scheduleAdWrapper,
                    matchScheduleList: schedule?.scheduleAdWrapper?.matchScheduleList?.filter((match: any) =>
                        match?.seriesCategory?.toLowerCase() === type?.toLowerCase()
                    ),
                },
            }))?.filter((schedule: any) => schedule?.scheduleAdWrapper?.matchScheduleList?.length > 0); // Filter out empty schedules
                    ...
                    <div className="flex flex-col justify-center items-center md:gap-10 min-h-full mb-4 sm:mb-8 lg:mb-16 min-w-full lg:max-w-[260px] lg:min-w-[200px]">
                        ...
                        <div className="lg:flex flex-col hidden gap-10 dark:bg-[#45474a80] bg-white/60 dark:text-[#E6E6DD] min-h-full py-4 px-6 rounded-2xl">
                            <div className="flex flex-wrap w-full gap-3">
                                <Tag name="All" onClick={() => handleTagClick('All')} />
                    ...
                    <div className="flex-grow">
                        <Tabs defaultValue="live" className="w-full">
                            ...
                            <TabsContent value="live" className="flex w-full flex-col gap-10">
                                <DayMatchList data={filteredData} />
